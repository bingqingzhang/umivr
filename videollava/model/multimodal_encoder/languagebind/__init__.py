...
from .tokenization_image import LanguageBindImageTokenizer
...
class LanguageBind(nn.Module):
    def __init__(self, clip_type=('thermal', 'image', 'video', 'depth', 'audio'), use_temp=True, cache_dir='./cache_dir', lora_flag=False):
        ...
        for c in clip_type:
            ...
            model = model_dict[c].from_pretrained(pretrained_ckpt, cache_dir=cache_dir, lora_flag=lora_flag)
...
class LanguageBindImageTower(nn.Module):
    ...
    def load_model(self):
        ...
        self.retrieval_image_proj = model.visual_projection
    ...
    @torch.no_grad()
    def forward(self, images, return_val=None):
        if type(images) is list:
            ...
            if return_val == "all_with_logit":
                return image_forward_outs, image_features, self.logit_scale
...
class LanguageBindVideoTower(nn.Module):
    ...
    ############################################################
    def load_model(self):
        model = LanguageBindVideo.from_pretrained("LanguageBind/LanguageBind_Video",cache_dir=self.cache_dir,lora_flag=True)
        ...
        self.video_tower_text_encoder = model.text_model
        self.retrieval_text_proj = model.text_projection
        self.retrieval_video_proj = model.visual_projection
        self.logit_scale = model.logit_scale
        self.is_loaded = True
    ...
    @torch.no_grad()
    def forward(self, videos, return_val=None):
        if type(videos) is list:
            ...
            if t != 8:
                ...
                special_t = None
            video_forward_outs = self.video_tower(videos.to(device=self.device, dtype=self.dtype), output_hidden_states=True, special_t=special_t)
            video_features = self.feature_select(video_forward_outs).to(videos.dtype)
            if return_val == "all":
                return video_forward_outs, video_features
            if return_val == "all_with_logit":
                return video_forward_outs, video_features, self.logit_scale
